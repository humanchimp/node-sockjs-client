...
SockJS.prototype._try_next_protocol = function(close_event) {
    ...
    while(1) {
        var protocol = this.protocol = this._protocols.shift();
        if (!protocol) {
            return false;
        }

        if (!SockJS[protocol] ||
              !SockJS[protocol].enabled(this._base_url)) {
            this._log('Skipping transport:', protocol);
        } else {
            var roundTrips = SockJS[protocol].roundTrips || 1;
            var to = ((this._rto || 0) * roundTrips) || 5000;
            this._transport_tref = helpers.delay(to, timeoutFunction);

            var connid = helpers.random_string(8);
            var trans_url = this._base_url + '/' + this._server + '/' + connid;
